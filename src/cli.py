...
def main():
    ...
    blast2bed_parser = sub_parsers.add_parser(
        "blast2bed", help="Convert BLAST output to BED and predict gene loci"
    )

    # Blast2bed arguments
    blast2bed_parser.add_argument(
        "input",
        metavar="input",
        type=lambda x: is_valid_file(parser, x),
        help="BLAST file in tabular format (required)",
    )
    blast2bed_parser.add_argument(
        "-e",
        "--exons",
        ...
        required=False,
        default=0,
        help="Expected number of exons in the gene (default: 0)",
    )
    blast2bed_parser.add_argument(
        "-c",
        "--coverage",
        ...
        required=False,
        default=1.1,
        help="Proportion of gene that must be covered by a predicted locus (default: 1.1)",
    )
    blast2bed_parser.add_argument(
        "-l",
        "--locus_size",
        ...
        required=False,
        default=1000,
        help="Expected size of the locus in base pairs (default: 1000)",
    )

    # Assemble-exons parser
    assemble_exons_parser = sub_parsers.add_parser(
        "assemble-exons", help="Assemble gene from specified exons"
    )

    # Assemble-exons arguments
    assemble_exons_parser.add_argument(
        "-i",
        "--input",
        metavar="input",
        type=lambda x: is_valid_file(parser, x),
    ...
    assemble_exons_parser.add_argument(
        ...
        metavar="blastdb",
        required=True,
        help="Path to the BLASTdb (including name)",
    )
    assemble_exons_parser.add_argument(
        "-e",
        "--exons",
    ...
    assemble_locus_parser.add_argument(
        "-i",
        "--input",
        metavar="input",
        type=lambda x: is_valid_file(parser, x),
    ...
    assemble_locus_parser.add_argument(
        ...
        type=str,
        required=True,
        help="Name of the predicted locus",
    )
    assemble_locus_parser.add_argument(
        "-o",
        "--output",
